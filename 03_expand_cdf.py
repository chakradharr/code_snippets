...
def expand_dataset_dask(data, id_col='pme_reference_no', time_col='t', end_time=30):
    ...
    # Define the meta DataFrame structure
    meta = pd.DataFrame(columns=[id_col, time_col] + [col for col in data.columns if col not in [id_col, time_col]],
                        dtype=data.dtypes)

    grouped = data.groupby(id_col)
    results = grouped.apply(process_group, meta=meta)
    result = dd.from_pandas(pd.concat(results.compute()), npartitions=data.npartitions)
